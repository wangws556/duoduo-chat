...
namespace YoYoStudio.Controls.CustomWindow
{
    public abstract class ParentWindow<ActionType> : EssentialWindow<ActionType> where ActionType :struct
    {
        ...
        public event Action<object, EventArgs> WindowHeaderMouseLeftButtonDownEvent;

        protected void RaiseWindowHeaderMouseLeftButtonDownEvent(object sender, EventArgs e)
        {
            if (WindowHeaderMouseLeftButtonDownEvent != null)
            {
                WindowHeaderMouseLeftButtonDownEvent(sender, e);
            }
        }
    }
}
