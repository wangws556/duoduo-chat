...
function InitRoom(giftGroups, self, w, h, publicMsgCount, privateMsgCount, msgPerSecond) {
    ...
    giftPanel1 = $("#giftPanel1");
    giftPanel2 = $("#giftPanel2");
    giftExpandar = $("#giftExpander");
    rightDockPanel = $('#rightDockPanel');
    giftExpandarSpan = $("#giftExpanderSpan");
    ...
    //    mode:'vertical'
    //});
    giftExpandar.click(function () {
        giftPanel1.slideToggle(200, function () {
            giftPanel2.slideToggle(200, function () {
                if (giftPanel2.is(":visible")) {
                    userListTab.jqxTabs({ width: '100%', height: rightDockPanel.height() - 385 });
                    giftExpandarSpan.text("隐藏礼物");
                }
                else {
                    userListTab.jqxTabs({ width: '100%', height: rightDockPanel.height() - 165 });
                    giftExpandarSpan.text("显示礼物");
                }
            });
        });
    });
...
function Resize(w, h) {
    ...
    if (giftPanel2.is(":visible"))
        userListTab.jqxTabs({ width: '100%', height: h - $('#topPanel').height() - 325 });
    else
        userListTab.jqxTabs({ width: '100%', height: h - $('#topPanel').height() - 105 });
}
        ...
        if (u) {
            ...
            if (uid && uid > 0) {
                if (window.external.CanReceiveGift(uid)) {
                    var c = document.getElementById('giftInput').value;
                    if (c) {
                        var gc = parseInt(c);
                        if (gc && gc > 0) {
...
function scoreReady() {
    var score = parseInt($('#toExchangeScore').val());
    if(score == null || score < 1000)
    {
        alert("你的积分不足1000，满1000的积分可换金币");
        return;
    }
    $('#getMoney').text(score * parseInt(scoreToMoneyLabel.text()) / 100);
}

function exchangeOk() {
    var toExchange = parseInt($('#toExchangeScore').val());
    var availableScore = parseInt($('#availableScore').text());
    var getMoney = parseInt($('#getMoney').text());
    if (toExchange == null || toExchange < 1000) {
        alert("用来兑换的积分不足1000，满1000的积分可换金币");
        return;
    }
    if (toExchange)
    {
