...
namespace YoYoStudio.Client.ViewModel
{
    ...
    public partial class HallWindowViewModel : WindowViewModel
    {
        ...
        {
            base.LoadAsync();
            CallJavaScript("InitGlobalAdvPage", GlobalSlidesJson, GlobalScrollJson, GlobalAdvPicJson);
        }

        #region Json

        public string GlobalSlidesJson
        {
            get
            {
                var globalSlides = ApplicationVM.LocalCache.AllImages[BuiltIns.GlobalSlideAdvertisePic.Id].Values;
                JavaScriptSerializer js = new JavaScriptSerializer() { MaxJsonLength = int.MaxValue };
                return js.Serialize(globalSlides);
            }
        }

        public string GlobalScrollJson
        {
            get
            {
                var globalScrolls = ApplicationVM.LocalCache.AllImages[BuiltIns.GlobalCompanyAdvPics.Id].Values;
                JavaScriptSerializer js = new JavaScriptSerializer() { MaxJsonLength = int.MaxValue };
                return js.Serialize(globalScrolls);
            }
        }

        public string GlobalAdvPicJson
        {
            get
            {
                var globalPic = ApplicationVM.LocalCache.AllImages[BuiltIns.GlobalAdvertisePics.Id].Values;
                JavaScriptSerializer js = new JavaScriptSerializer() { MaxJsonLength = int.MaxValue };
                return js.Serialize(globalPic);
            }
        }
        ...
        public void EnterRoom(int roomId)
        {
			...
			if (roomVM != null && !string.IsNullOrEmpty(roomVM.ServiceIp))
			{
				roomVM.Enter();
			}
			else
        ...
        public void UpdateLoginInfo(string userid, string pwd, bool remember, string icon, bool autoLogin)
        {
            ...
            bool result = false;
            ApplicationVM.ProfileVM.Save(out result);
        }
