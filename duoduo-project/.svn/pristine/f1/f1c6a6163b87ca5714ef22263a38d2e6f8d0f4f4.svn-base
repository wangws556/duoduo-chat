...
namespace YoYoStudio.ManagementPortal.Controllers
{
    ...
    public class ImageController : BaseController
    {
        ...
        private ActionResult SaveAdvertise(Image img)
        {
            HttpPostedFileBase file = null;
            string result = YoYoStudio.Resource.Messages.UploadError;
            if (Request.Files.Count > 0)
            {
                ...
                file.InputStream.Position = 0;
                file.InputStream.CopyTo(new MemoryStream(data));
                Image image = new Image { Id = img.Id, OldId = img.Id, Name = name, Ext = ext, ImageType_Id = img.ImageType_Id, TheImage = data,Link = img.Link };
            ...
            return View("UploadImage", new ImageModel() { Id = img.Id, OwnerId = img.Id, OldId = img.Id, Message = result });
        }
        ...
        public ActionResult SaveImage(string ownertype, int? ownerid, int? imageid, int? oldId)
        {
            ...
            if (ownerid.HasValue)
            {
                ModelEntity mEntity = GetEntity(ownertype, new int[] { ownerid.Value });
                ImagedEntity entity = mEntity as ImagedEntity;
                if (entity != null)
                {
                ...
                {
                    Image img = mEntity as Image;
                    if (img != null)
                    {
                        return SaveAdvertise(img);
                    }
                }
            }
