...
namespace YoYoStudio.DataService.Library
{
    ...
    public partial class DataService : WcfService, IDataService, IErrorHandler, IServiceBehavior
    {
        ...
        public void UpdateImage(int appid, int userId, string token, Image image)
        {
            ...
            {
                CheckToken(appid, userId, token);
                if (image.TheImage == null)
                {
                    Image img = new Image{Id = image.Id};
                    modelAccesser.Get<Image>(img);
                    image.TheImage = img.TheImage;
                }
        ...
        [OperationBehavior]
        public List<Image> GetImagesWithBody(int appid, int userId, string token, string condition = "", int start = -1, int count = -1)
        {
            try
            {
                CheckToken(appid, userId, token);
