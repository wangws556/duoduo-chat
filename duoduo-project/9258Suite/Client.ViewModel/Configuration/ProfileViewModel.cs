...
namespace YoYoStudio.Client.ViewModel
{
    ...
    public partial class ProfileViewModel : TitledViewModel
    {
        ...
        {
            Title = YoYoStudio.Resource.Text._9258ChatApplication;
        }
        ...
        private void PrepareSerialize()
        {
            foreach (LoginEntryViewModel vm in LoginEntryVMs)
            {
                ...
                {
                    profile.LoginEntries.Add(new LoginEntry { UserId = vm.UserId, Password = vm.Password, Remember = vm.Remember, Icon = vm.IconPath });
                }
            }
            ...
            if (LastLoginVM != null)
            {
                if (profile.LastLoginEntry == null)
                {
                    if (LastLoginVM.Remember == false)
                        profile.LastLoginEntry = new LoginEntry { Password = string.Empty, Remember = LastLoginVM.Remember, UserId = LastLoginVM.UserId, Icon = LastLoginVM.IconPath };
                    else
                    {
                        profile.LastLoginEntry = new LoginEntry { Password = LastLoginVM.Password, Remember = LastLoginVM.Remember, UserId = LastLoginVM.UserId, Icon = LastLoginVM.IconPath };
                    }
                }
                else
                {
                    ...
                    if (LastLoginVM.Remember == false)
                        profile.LastLoginEntry.Password = string.Empty;
                    else
                        profile.LastLoginEntry.Password = LastLoginVM.Password;
                    profile.LastLoginEntry.Remember = LastLoginVM.Remember;
                }

                LoginEntry entry = profile.LoginEntries.Find(r => r.UserId == profile.LastLoginEntry.UserId);
                if (entry != null)
                {
                    if (LastLoginVM.Remember == false)
                        entry.Password = string.Empty;
                    else
                        entry.Password = LastLoginVM.Password;
                    entry.Remember = LastLoginVM.Remember;
                }
            }
