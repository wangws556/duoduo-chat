        ...
        private void GiftSentEventHandler(RoomMessage msg,bool broadcastMsg)
        {
            switch (msg.GiftResult)
            {
                case SendGiftResult.Succeed:
                    ...
                    {
                        string header = "<img style='width:16px; height:16px' title='" + sender.RoleVM.Name + "' src='" + sender.RoleVM.ImageVM.StaticImageFile + "'><u style='color:gold;margin-right:10px'><span  onclick='window.external.SelectUser(" + sender.Id + ")'" +
                                    " oncontextmenu='window.external.SelectUser(" + sender.Id + ")'/>" + sender.NickName + "(" + sender.Id + ")" + "</span></u></img> 送给 " +
                                    "<img style='width:16px; height:16px' title='" + receiver.RoleVM.Name + "' src='" + receiver.RoleVM.ImageVM.StaticImageFile + "'><u style='color:purple;margin-left:10px;margin-right:10px'><span onclick='window.external.SelectUser(" + receiver.Id + ")'" +
                        ...
                        {
                            htmlmsg += "<span>" + msg.Count + gift.Unit + gift.Name + "<img style='width:30px; height:30px' src='" + gift.ImageVM.DynamicImageFile + "'/></span>";
                            msg.Content = htmlmsg;
                            ShowHornMsg(msg);
                        }
                        ...
                        if (msg.Count >= gift.RunWay)
                        {
                            ...
                            htmlmsg += "<img style='margin-left:20px;margin-right:20px; width:120px; height:60px ' src='" + gift.ImageVM.DynamicImageFile + "'/>";
                        ...
                        if (!broadcastMsg)
                        {
                            ...
                            htmlmsg = header;
                            htmlmsg += "<span>一" + gift.Unit + gift.Name + "<img style='width:60px; height:60px' src='" + gift.ImageVM.DynamicImageFile + "'/>, 共";
